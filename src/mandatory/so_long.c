...
/*   Updated: 2023/11/24 19:41:02 by patatoss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv)
{
	...
	if (is_all_1(game) == 1)/*  */
		perror_free_str_map_fd(NULL, game.map, "Invalid map.\n", 0);
	...
	if (game.total_of_collectibles == 0)
		perror_free_str_map_fd(NULL, game.map, "Error. The map doesn't have all the items needed.\n", 0);
	...
	if (check_path(path_checker, game.player.x / 64, game.player.y / 64, 0) != game.total_of_collectibles)/*  */
		perror_free_str_map_fd(NULL, game.map, "Invalid path.\n", 0);/*  */
